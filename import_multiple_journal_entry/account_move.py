...
class gen_journal_entry(models.TransientModel):
		...
		def create_import_move_lines(self, values):
			 ...
			 if values.get('full_reconcile_id') != '':
				 reconcile = self.env['account.full.reconcile'].create({'name' : values.get('full_reconcile_id')})
				 values.update({'full_reconcile_id' : reconcile.id })
		...
		def import_move_lines (self):
			 if  self.import_option == 'csv':
					 keys = ['number','date','ref','journal','name','partner','analytic_account_id', 'account_code', 'date_maturity', 'debit', 'credit', 'amount_currency', 'currency', 'full_reconcile_id']
					 ...
					 for key in data1.keys():
							 ...
							 for val in values:
									...
									if journal_search:
										move1 = move_obj.search([('name','=',val.get('number')),
																		('date','=',val.get('date')),
																		('ref', '=', val.get('ref')),
										...
										if move1:
												...
												move = move_obj.create({'name':val.get('number') or False,
							 ...
							 for row_no in range(sheet.nrows):
									 ...
									 if row_no <= 0:
											 ...
											 date  = False
											 if line[8] != '' and line[1] !='':
													date = str(xlrd.xldate.xldate_as_datetime(int(float(line[8])), workbook.datemode))
													main_date = str(xlrd.xldate.xldate_as_datetime(int(float(line[1])), workbook.datemode))
											 values =  {'number':line[0],
														...
														'debit': line[9],
														'credit': line[10],
														'amount_currency': line[11],
														'currency': line[12],
														'full_reconcile_id': line[13],
