...
class BonusRuleInput(models.Model):
    ...
    # nbj_taken_for_alloc = fields.Float()
    ...
    def action_payslip_done(self):
        for payslip in self:
            ...
            if payslip.employee_id.ir_changed == 1:
                payslip.employee_id.ir_changed = 0
    ...
    def compute_sheet(self):
        for payslip in self:
            if payslip.state == "draft":
                ...
                # nbj_taken_for_alloc = 0
                # if payslip.contract_id.nbj_pris != 0:
                #    nbj_taken_for_alloc = payslip.contract_id.nbj_pris

                lines = [(0, 0, line) for line in self.get_payslip_lines(contract_ids, payslip.id)]
                payslip.write({'line_ids': lines, 'number': number, 'nbj_taken_for_alloc': nbj_taken_for_alloc})
    ...
    def get_payslip_lines(self, contract_ids, payslip_id):
        for record in self:
            ...
            # # Computing Wage and other based on Net salary on playslip
            # percentage_total = 0
            # fixed_amount = 0
            # for ru in sorted_rules:
            #     if ru.category_id.code == 'DED':
            #         if ru.amount_select == 'percentage':
            #             percentage_total = percentage_total + ru.amount_percentage
            #         if ru.amount_select == 'fix':
            #             fixed_amount += ru.amount_fix
            #     if ru.category_id.code == 'INDM ':
            #         if ru.amount_select == 'percentage':
            #             percentage_total = percentage_total - ru.amount_percentage
            #         if ru.amount_select == 'fix':
            #             fixed_amount -= ru.amount_fix
            # sorted_rules.search([('category_id', '=', 'Basic')]) \
            #     .write({'amount_fix': ((record.net_salary + fixed_amount) / (1 - (percentage_total / 100)))})
...
class HrPayslipRunExtend(models.Model):
    ...
    def pay_payslip(self):
        ...
        for slips in self.slip_ids:
            if slip.state != 'done':
                slip.contract_id._get_droit(slip.date_from)
