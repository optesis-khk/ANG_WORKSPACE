...
class ResConfigSettingsInherit(models.TransientModel):
    ...
    payroll_chart_template = fields.Many2one('payroll.chart.template', 'Payroll Template')

    @api.multi
    def set_values(self):
        super(ResConfigSettingsInherit, self).set_values()
        """ install a chart of accounts for the given company (if required) """
        if self.payroll_chart_template and self.payroll_chart_template != self.company_id.payroll_chart_template:
